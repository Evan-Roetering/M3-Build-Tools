...
&emsp;&emsp;&emsp; NOTE 2: Insane places a number of lipids based on the ratio of input numbers, to ensure proper lipid counts X and Y should both be equal to the box length value described by:
   
$$
L = \sqrt{A*\sum_{lipid}^{lipids}NUM_{lipid}}
$$  
...
6. run it with `./min_eq.start`
7. after minimization and equilibration edit `production.start` with email and number of cores (64 is a good starting point for medium systems, but may be unstable for small systems or slow for large ones)
8. run it with `./production.start`
9. Visualize double leaflet simulation in vmd using files generated in the `asymmetric-system/double-\<upper/lower\>/martiniglass` directory
10. If undulations appear to be present in visualizations use `./undulation_area.sh` in the `asymmetric-system/double-\<upper/lower\>/analysis` directory, otherwise use `flat_area.sh` edit the email address in the script before running
11. Check outputs for area metrics
12. Based on area metrics, quantify the number of lipids per leaflet to cover the same area
13. navigate to the `asymmetric-system/asymmetric/system` build directory
14. construct lipid topology with `./make_toppar.sh`
15. construct asymmetrical bilayer using insane with the same syntax as earlier:  
...
16. name system and make `system.top` file using `./make_system_top.sh "\<system name\>"`
17. run `gmx_mpi grompp -f ions.mdp -c membrane.gro -p system.top -o ions.tpr -maxwarn 1`
18. run `gmx_mpi genion -s ions.tpr -o system_ions.gro -p system.top -neutral -pname NA`  
&emsp; select waters as the solvent  
19. make MEMBRANE and SOLUTE index with `gmx_mpi make_ndx -f system_ions.gro -o index.ndx`  
...
20. run `gmx_mpi editconf -f system_ions.gro -o step5_charmm2gmx.pdb` to save in the same output format as CHARMMM-GUI
21. navigate back one directory to `asymmetric-system/asymmetric` and edit `min_eq.start` with email and cpus
22. run `./min_eq.start`
23. after minimization and equilibration, edit `production.start` with email, cores, sim_steps, and n_runs
