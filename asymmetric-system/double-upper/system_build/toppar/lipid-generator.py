...
"""
lipid-itp-generator-Martini3 creates a customized Martini lipid topologies for Martini 3 lipids, use lipid-itp-generator-Martini3-02.py -h for description.
Modified by Evan Roetering from original script by Ingolfsson et al
"""

__author__  = "Evan Roetering, Helgi I. Ingolfsson, Kasper B. Pedersen, and Tsjerk A. Wassenaar"
__status__  = "Development"
__version__ = "M3.l02"
__email__   = "eroeteri@umd.edu"
...
For increased clarity and easy of modififcation the majority of the lipid paramters are defined as  
...
Linkers (-allink):
  ...
  linker beads types supported:
    ...
    L = Plasmalogen glycerol - normally G L -> GL1 PL2 with bead types SN4a and SN4as, modified to support L G -> PL1 GL2
    E = Ether glycerol       - normally E E -> ET1 ET2 with bead type SN3a, modified to support E G - > ET1 GL2
    A = Sphingosine backbone - normally A1 A2 -> OH1 AM2 with bead type SP1 and SP2
    Y = Glycerol backbone for lysolipids - normally G Y, but supports E Y and L Y -> GL1/ET1/PL1 LY2 where bead types are SN4a/SN4as/SN3a and TN6
...
linkerMapp = {
    ...
    "Y":   ['TN6',  'LY?',  '0', 'GL' ],  # L = glycerol linked to hydrogen 
...
if (linkersArray[0] == "G"): # first linker is GLY
    ...
    elif (linkersArray[1] == "Y"): # second linker is Y so this is a lysolipid
        # Note G Y - used for lysolipids
        linkerType = "glyc"
        lipidType = "def" # _def is default parameters (not specifid to lipid type)
    ...
    if (linkersArray[1] == "G"): # second linker is G so this is an SN1 ether linker
        # Note E G - used for SN1 ether lipids
        linkerType = "ether"
        lipidType = "def" # _def is default parameters (not specifid to lipid type)
    elif (linkersArray[1] == "Y"): # second linker is Y so this is a lysoetherlipid
        # Note E Y - used for lysoetherlipids
        linkerType = "ether"
        lipidType = "def" # _def is default parameters (not specifid to lipid type)
    else: # assume normal E E - used for ether lipids 
        linkerType = "ether"
        lipidType = "def" # _def is default parameters (not specifid to lipid type) 
elif (linkersArray[0] == "L"): # Plasmalogen lipid linker
    if (linkersArray[1] == "G"): # second linker is G so this is an SN1 plasmalogen linker
        # Note G L - used for plasmalogens
        linkerType = "plasm"
        lipidType = "def" # _def is default parameters (not specifid to lipid type)
    elif (linkersArray[1] == "Y"): # second linker is Y so this is a lysoplasmalogenlipid
        # Note E Y - used for lysoplasmalogenlipids
        linkerType = "plasm"
        lipidType = "def" # _def is default parameters (not specifid to lipid type)
    else: # A linker
        print(f"Linker type {linkersArray[1]} not supported with {linkersArray[0]}", file=sys.stderr)
        sys.exit()
else: # A linker
    ...
    for i in range(len(headsArray)):
        ...
        if index > 1: # link head beads
            if headsArray[i-1] == 'PS1' and headsArray[i] == 'PS2':
                ...
                print(f"added angle: {anglesArray[-1]}")
            else:
...
for linkerBeadIndex in range(0, len(linkersArray)):
    ...
    if (cLinker == "G" or cLinker == "E" or cLinker == "L" or cLinker == "Y"): # GLY (E and L are gly but for plasmalogen or ether lipids)
        if len(tailsArray[linkerBeadIndex]) == 1 and tailsArray[linkerBeadIndex][0]=='-':  # No tail given for current linker bead so free OH group Na changed to P1
            ...
            #print("Not yet supported @TODO add first if", file=sys.stderr)
            #sys.exit()
            lyso=True
            ...
            print("Not yet supported @TODO add second if", file=sys.stderr)
            sys.exit()
...
print('\n#ifdef BILAYER_LIPIDHEAD_FC', file=itpFile)
print('    [ position_restraints ]', file=itpFile)
print('    ;to fix Z postion of head grop in bilayer simulation', file=itpFile)
print('      2     1.0      0.0      0.0     BILAYER_LIPIDHEAD_FC', file=itpFile)
print('#endif', file=itpFile)
